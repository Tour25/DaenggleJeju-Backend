...
class FootprintCreateSerializer(serializers.Serializer):
    ...
    entryStatusDetail = serializers.CharField(
        ...
        help_text="entryStatus=detail(상세 입력) 일 때 5~20자"
    )
    ...
    welcome = serializers.ChoiceField(
        choices=[5, 4, 3, 2, 1],
        help_text="5(매우 환영) · 4(편안) · 3(보통) · 2(조금 어려움) · 1(아쉬움)",
    )
...
class MyFootprintListQuery(serializers.Serializer):
    contentTypeId = serializers.IntegerField(required=False,help_text="32=숙박, 39=음식점, 12=관광지, 28=레포츠, 38=쇼핑")
